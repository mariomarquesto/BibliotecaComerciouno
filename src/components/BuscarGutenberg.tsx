...
  // Por favor, reemplaza esta clave de ejemplo con tu clave de API real de Google Cloud.
  const GOOGLE_BOOKS_API_KEY = 'AIzaSyCEfV95URudokpaYbmZBOLHWF58392v734';
...
    if (!GOOGLE_BOOKS_API_KEY || GOOGLE_BOOKS_API_KEY === 'AIzaSyCNgS-oG5b8H_D8hUi9S_iAjQaExpN1Y5Q') { 
      console.warn("ADVERTENCIA: La clave de Google Books no está configurada o es el placeholder. La búsqueda de Google Books no funcionará. Por favor, reemplaza la clave de API con una válida.");
      return [];
    }
   
    // CORRECCIÓN CLAVE: Ajustar la sintaxis de la URL de búsqueda de Google Books
...
      queryParams += `intitle:${encodeURIComponent(trimmedSearchTerm)}`;
    }
    if (category) {
      if (queryParams) {
        queryParams += `+`; // Usa "+" para concatenar búsquedas en la API de Google Books
      }
      queryParams += `subject:${encodedCategory}`;
    }
    
    if (!queryParams) {
      return [];
...
    const googleBooksApiUrl = `https://www.googleapis.com/books/v1/volumes?q=${queryParams}&langRestrict=${encodedLanguage}&key=${GOOGLE_BOOKS_API_KEY}&maxResults=20`;
